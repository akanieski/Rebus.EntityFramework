...
public class EntityFrameworkSagaSnapshotStorage(RebusDbContext db) : ISagaSnapshotStorage
{
    public async Task Save(ISagaData sagaData, Dictionary<string, string> sagaAuditMetadata)
    {
        db.ChangeTracker.Clear();
        var existingSnapshot = await db.SagaSnapshots.SingleOrDefaultAsync(s => s.Id == sagaData.Id && s.Revision == sagaData.Revision);
        ...
        else if (sagaData.Revision != 0 && existingSnapshot != null && existingSnapshot.Revision == sagaData.Revision)
        {
            throw new RebusApplicationException($"Attempted to save a saga [{sagaData.Id}] snapshot with the same revision [{sagaData.Revision}] as the existing snapshot!");
        }
